  ...
  const stats = useMemo(() => {
    ...
    const avgProgress =
      total === 0
        ? 0
        : Math.round(
            perGoal.reduce((s, g) => s + (g.__progress || 0), 0) / total
          );
    ...
    const nearest =
      futureEnds.length > 0 ? futureEnds[0].toISOString().split("T")[0] : null;
    ...
    const last7Days = Array.from({ length: 7 }, (_, i) => {
      ...
      const completed = perGoal.filter((g) =>
        (g.completedDays || []).includes(iso)
      ).length;
    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-800 p-6">
        <div className="flex items-center gap-3 mb-6">
          <button
            onClick={() => setShowGoalStats(false)}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow p-6">
            <h3 className="text-lg font-bold mb-4">
              {t("تطور الأهداف الأسبوعي", "Weekly Goal Progress")}
            </h3>
            <ResponsiveContainer width="100%" height={200}>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow p-6 mt-6">
          <h3 className="text-lg font-bold mb-4">
            {t("الخط الزمني للأهداف", "Goals Timeline")}
          </h3>
